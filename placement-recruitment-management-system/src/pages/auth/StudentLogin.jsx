...
export default function StudentLogin() {
  ...
  const fillDemoCredentials = () => {
    setValue('email', 'student.demo@indus.edu');
  ...
  return (
    ...
    >
      <div className="text-center mb-4">
        <div className="d-inline-flex align-items-center justify-content-center bg-primary bg-opacity-10 text-primary rounded-circle mb-3" style={{ width: '54px', height: '54px' }}>
          <Mail size={26} />
        </div>
        <h3 className="fw-bold text-slate-900 mb-1.5" style={{ fontSize: '1.5rem', letterSpacing: '-0.02em' }}>Student Sign In</h3>
        <p className="text-secondary mb-0" style={{ fontSize: '0.875rem' }}>
          Welcome back! Enter your college credentials below.
        </p>
        ...
        {isDemoMode && (
          <div className="mt-3">
            <button
              ...
              className="btn btn-sm btn-outline-primary rounded-pill px-3 py-1 fw-medium d-inline-flex align-items-center gap-1.5"
              style={{ fontSize: '0.78rem' }}
            >
              <span>Auto-Fill Demo: student.demo@indus.edu</span>
            </button>
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="d-flex justify-content-between align-items-center mb-4" style={{ fontSize: '0.85rem' }}>
          <div className="form-check">
            <input className="form-check-input" type="checkbox" id="stdRememberMe" style={{ cursor: 'pointer' }} />
            <label className="form-check-label text-secondary fw-medium" htmlFor="stdRememberMe" style={{ cursor: 'pointer', userSelect: 'none' }}>
              Remember my device
            </label>
          </div>
        </div>

        <button type="submit" disabled={loading} className="btn btn-premium btn-premium-primary w-100 py-2.5 mb-3 d-flex align-items-center justify-content-center gap-2">
          {loading ? (
            <>
        ...
        <div className="text-center mt-4" style={{ fontSize: '0.875rem' }}>
          ...
          <Link to="/student/signup" className="text-primary fw-semibold text-decoration-none">
